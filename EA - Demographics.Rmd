...
treligion <- tbirthcountry <- as.data.frame(table(demographics$religion)/nrow(demographics)*100); colnames(treligion) <- c('Religion', 'Percent')
...
sex <- ggplot(demographics) + geom_bar(aes(x = year, fill = sex), position = position_dodge(preserve = 'single')) + labs(title = 'Respondent Sex Distribution', y = 'Number of Respondents', x = 'Year', fill = 'Sex')
qual <- ggplot(demographics) + geom_bar(aes(x = year, fill = qualifications), position = position_dodge(preserve = 'single')) + labs(title = 'Respondent Qualification Distribution', y = 'Number of Respondents', x = 'Year', fill = 'Qualifications')
country <- ggplot(demographics) + geom_bar(aes(x = year, fill = birth_country), position = position_dodge(preserve = 'single')) + labs(title = 'Respondent Birth Country Distribution', y = 'Number of Respondents', x = 'Year', fill = 'Birth Country') + scale_fill_discrete(labels = c('Australia', 'Non-English Speaking', 'English-Speaking'))
religion <- ggplot(demographics) + geom_bar(aes(x = year, fill = religion), position = position_dodge(preserve = 'single')) + labs(title = 'Respondent Religion Distribution', y = 'Number of Respondents', x = 'Year', fill = 'religion')
social <- ggplot(demographics) + geom_bar(aes(x = year, fill = social_class), position = position_dodge(preserve = 'single')) + labs(title = 'Respondent Class Distribution', y = 'Number of Respondents', x = 'Year', fill = 'Class')
living <- ggplot(demographics) + geom_bar(aes(x = year, fill = living_status), position = position_dodge(preserve = 'single')) + labs(title = 'Respondent Living Status Distribution', y = 'Number of Respondents', x = 'Year', fill = 'Living Status Type')
income <- ggplot(demographics) + geom_bar(aes(x = year, fill = annual_income), position = position_dodge(preserve = 'single')) + labs(title = 'Respondent Annual Income Distribution', y = 'Number of Respondents', x = 'Year', fill = 'Annual Income Bracket')

figuredemo1 <- ggarrange(sex, qual, country, religion, social, living, income, nrow = 8)
...
income2 <- income <- ggplot(demographics) + geom_bar(aes(x = annual_income, fill = annual_income)) + labs(title = 'Respondent Annual Income Distribution', y = 'Number of Respondents', x = element_blank(), fill = 'Annual Income Bracket') + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
...
```{r}
map("worldHires", ylim = c(-45,-11 ), xlim = c(110,160 ), fill = TRUE, col = "lightgrey", mar = c(4.5, 4, 1, 1))
map.axes()

spplot(Freq_by_Suburb, z = 'Frequency', ylim = c(-45,-11 ), xlim = c(110,160 ))
```
